...
int main (int argc, char **argv){
                    ...
                    while (fim == 0) {
                        ...
                        if (primeiro == 1) {
                            ...
                            if (ct->leituraIndividual == 1) {
                                ...
                                MPI_Recv(&completedIndexes, 1, MPI_CHAR, i, i, MPI_COMM_WORLD, &status);
                                ...
                                while (lido == 0) {
                                    #pragma omp critical (MPI_COMM_WORLD)
                                    {
                                        if (ler == 0) {
                                            ...
                                            MPI_Ssend(&completedIndexes, 1, MPI_CHAR, i, i, MPI_COMM_WORLD);
                                            lido = 1;
                                            MPI_Recv(&completedIndexes, 1, MPI_CHAR, i, i, MPI_COMM_WORLD, &status);
                            ...
                            while (gravado == 0) {
                                #pragma omp critical (MPI_COMM_WORLD)
                                {
                                    if (gravar == 0) {
                                        ...
                                        MPI_Ssend(&completedIndexes, 1, MPI_CHAR, i, i, MPI_COMM_WORLD);
                                        gravado = 1;
                                        MPI_Recv(&relogio[i].tempoR, 1, MPI_FLOAT, i, i, MPI_COMM_WORLD, &status);
                                        MPI_Recv(&relogio[i].tempoF, 1, MPI_FLOAT, i, i, MPI_COMM_WORLD, &status);
                                        MPI_Recv(&relogio[i].tempoW, 1, MPI_FLOAT, i, i, MPI_COMM_WORLD, &status);
                        ...
                        if (blocks != 0) {
                            MPI_Ssend(&node[i].li, inteiro, MPI_INT, i, i, MPI_COMM_WORLD);
                            MPI_Ssend(&node[i].lf, inteiro, MPI_INT, i, i, MPI_COMM_WORLD);
                            primeiro = 1;
    ...
    //INICIO DOS NODES
        ...
        while (stop==0) {
            ...
            MPI_Recv(&node[rank].li, inteiro, MPI_INT, 0, rank, MPI_COMM_WORLD, &status);
            ...
            if (node[rank].li == -101) {
                ...
                MPI_Recv(&node[rank].lf, inteiro, MPI_INT, 0, rank, MPI_COMM_WORLD, &status);

                PPMThread* thread;

                if (ct->leituraIndividual == 1) {
                    if (ct->debug >= 1) printf("Node solicita entrada na fila de leitura: %d\n", rank);
                    MPI_Ssend(&completedIndexes, 1, MPI_CHAR, 0, rank, MPI_COMM_WORLD);

                    //AGUARDA AUTORIZACAO DO RANK 0
                    //PARA LER
                    MPI_Recv(&completedIndexes, 1, MPI_CHAR, 0, rank, MPI_COMM_WORLD, &status);
                ...
                if (completedIndexes == 'W') {
                    ...
                    MPI_Ssend(&relogio[rank].tempoR, 1, MPI_FLOAT, 0, rank, MPI_COMM_WORLD);
                    MPI_Ssend(&relogio[rank].tempoF, 1, MPI_FLOAT, 0, rank, MPI_COMM_WORLD);
                    MPI_Ssend(&relogio[rank].tempoW, 1, MPI_FLOAT, 0, rank, MPI_COMM_WORLD);
