                        ...
                        if (primeiro == 1) {
                            ...
                            while (gravado == 0) {
                                #pragma omp critical
                                {
                                    if (gravar == 0) {
                                        gravar = 1;
                                        completedIndexes[i] = -303;
                                        if (ct->debug >= 1) printf("Server[%d] permite node gravar: %d\n", tServer, i);
                                        MPI_Ssend(&completedIndexes[i], inteiro, MPI_INT, i, 05, MPI_COMM_WORLD);
                                        gravado = 1;
                                        MPI_Recv(&completedIndexes[i], inteiro, MPI_INT, i, 15, MPI_COMM_WORLD, &status);
        ...
        while (stop==0) {
            ...
            if (node[rank].li == -101) {
                ...
                if (completedIndexes[rank] == -303) {
                    ...
                    MPI_Ssend(&completedIndexes[rank], inteiro, MPI_INT, 0, 15, MPI_COMM_WORLD);
